...
def test_fullcase200_ef_20k_demo_sbatch_targets_current_env_and_compile():
    ...
    assert '"hybrid_muon_stage_two_lr_factor"' in text
    assert '"hybrid_muon_match_rms_coeff"' in text
    assert '"hybrid_muon_magma_lite"' in text
    assert '"hybrid_muon_magma_initial_score"' in text
    assert '"hybrid_muon_magma_warmup_steps"' in text
    assert '"hybrid_muon_magma_bypass_first_step"' in text
    ...
    assert '--hybrid_muon_stage_two_lr_factor="${HYBRID_MUON_STAGE_TWO_LR_FACTOR}"' in text
    assert "MACE_OC20NEB_HYBRID_MUON_TACE_MODULE_INCLUDE" in text
    assert "MACE_OC20NEB_HYBRID_MUON_TACE_MODULE_LR_SCALE" in text
    assert '"hybrid_muon_tace_module_include"' in text
    assert '"hybrid_muon_tace_module_lr_scale"' in text
...
def test_summarize_fullcase200_ef20k_matrix_reports_case_config_and_metrics(tmp_path):
    ...
    assert row["hybrid_muon_tace_module_include"] == "interactions.*"
    assert row["hybrid_muon_tace_module_lr_scale"] == 0.25
    assert row["hybrid_muon_lr_factor"] == 0.1
    assert row["hybrid_muon_stage_two_lr_factor"] == 0.25
    assert row["hybrid_muon_lr_scale_mode"] == "match_rms"
    assert row["hybrid_muon_match_rms_coeff"] == 0.18
    assert row["hybrid_muon_magma_lite"] == "True"
    assert row["hybrid_muon_magma_initial_score"] == 0.5
    assert row["hybrid_muon_magma_warmup_steps"] == 3
    assert row["hybrid_muon_magma_bypass_first_step"] == "True"
...
def test_summarize_fullcase200_ef20k_matrix_compares_cueq_adamw_to_module_muon(tmp_path):
    summarizer = load_script("summarize_fullcase200_ef20k_matrix.py")
    root = tmp_path / "matrix_cueq_module_muon"
    manifest = {
        "batch_size": 8,
        "target_steps": 20000,
        "train_size": 5000,
    ...
    for case_name, epoch_seconds, mae_e, mae_f, fb_memory in (
        ...
        ("cueq_hybrid_muon_module", 44.0, 150.06, 36.29, 10156),
    ):
        case_dir = root / case_name
        log_dir = case_dir / "logs"
        log_dir.mkdir(parents=True)
    ...
    comparison = next(
        ...
        and item["candidate_case"] == "cueq_hybrid_muon_module"
    )
    assert comparison["mae_e_delta_mev_atom"] == pytest.approx(116.36)
    assert comparison["mae_f_delta_mev_a"] == pytest.approx(-17.59)
    assert comparison["mae_f_ratio"] == pytest.approx(36.29 / 53.88)
