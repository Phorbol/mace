...
def test_edge_force_compiled_loss_cache_key_separates_loss_kind():
    ...
    forces_key = wrapper._cache_key(
        batch=batch,
        input_names=input_names,
        data_dict=data_dict,
        loss_fn=WeightedForcesLoss(),
    )
    l1l2_key = wrapper._cache_key(
        batch=batch,
        input_names=input_names,
        data_dict=data_dict,
        loss_fn=WeightedEnergyForcesL1L2Loss(),
    )

    assert forces_key != l1l2_key
    assert ("compiled_loss", "weighted_forces") in forces_key[-2]
...
def test_edge_force_compiled_loss_cache_entries_separate_loss_kind():
    ...
    prepared.compiled_force_training_loss(
        batch=batch,
        loss_fn=WeightedForcesLoss(),
        output_args={"forces": True, "virials": False, "stress": False},
    )
    ...
    cache_keys = list(prepared.cache.keys())
    assert any(("compiled_loss", "weighted_forces") in key[-2] for key in cache_keys)
    assert any(
        ("compiled_loss", "weighted_energy_forces_l1l2") in key[-2]
...
def test_take_step_copies_compiled_parameter_clone_grads_to_wrapped_model_params():
    from mace.tools.train import take_step
    ...
    class WrappedCompiledModel(torch.nn.Module):
        def __init__(self):
            super().__init__()
        ...
        def compiled_force_training_loss(self, *, batch, loss_fn, output_args):
            del loss_fn, output_args
            clone = self.model.weight.detach().clone().requires_grad_(True)
            self.clone_seen = clone
            loss = clone * batch.x.sum()
            return loss, {
                "edge_force_compile": True,
    ...
    assert model.clone_seen.grad.item() == pytest.approx(1.0)
    assert model.model.weight.grad.item() == pytest.approx(1.0)
    assert model.model.weight.item() == pytest.approx(0.9)
