...
Use this when the goal is an optimizer comparison. The OC20NEB wrapper defaults to `SCHEDULER=WSD` and `LR_SCHEDULER_INTERVAL=auto`; in this branch `auto` means per-step updates for WSD, so the Adam and HybridMuon OC20NEB jobs above already used batch/update-level WSD with warmup. Use the standard Stage Two recipe when the goal is reproducing normal MACE training accuracy. Do not compare single-stage WSD HybridMuon against two-stage Adam and attribute the difference only to the optimizer.
...
- HybridMuon is compatible with the compile path, but the current RECIO/8k evidence still favors Adam for throughput on small MACE models. Short 20k gates showed worse force accuracy for conservative MACE-routed HybridMuon, while the stricter L1/C64 200k WSD gate below showed that conservative routing can recover Adam-like final force accuracy when trained long enough. The remaining concern is speed and routing coverage, not basic correctness. TACE-style slice routing is compatible and faster than conservative Muon in full Inductor, but job `620634` finished in `09:46` with MaxRSS `5152156K` and test `40.4` meV/atom / `261.2` meV/A, worse than Adam full Inductor (`09:25`, `30.6` / `202.6`) and worse in force than conservative MACE-Muon (`11:17`, `36.1` / `223.2`). Keep TACE-style routing as an explicit ablation; do not make it the default without a changed scheduler/lr recipe and multi-seed evidence.

- Larger-model and cross-dataset gates sharpen the current boundary. The strict RECIO L2/C128 two-seed 200k WSD gate used Adam, full CUEQ, full-Inductor edge-force compile, TF32, AMP none, batch size 8, and seeds 123/789. Compile completed in 01:50:06 and 01:36:06 versus matched eager 02:07:07 and 02:08:12, giving about 1.24x mean wall-clock speedup. Compile health was clean for both seeds: one initial compile per run, fallbacks=0, runtime_recompiles=0, hot-step means 26.28 and 22.86 ms/batch. Final test metrics stayed close to eager: seed 123 eager 50.3 / 117.5, compile 50.3 / 118.5; seed 789 eager 47.9 / 114.8, compile 49.8 / 115.4. MaxRSS increased from about 4.28-4.29e6 K eager to 6.64-6.70e6 K compile. This removes the larger-model multi-seed parity gap for the recommended Adam path, but it also confirms that current V100/MACE/CUEQ compile speedup is moderate rather than DPA4-like 3x. OC20NEB FPS L1/C64 remains the cross-dataset gate below.
...
- The matching RECIO L1/C64 200k WSD eager control, SAI job `621821`, completed in `01:40:51` with MaxRSS `3.58 GB`. It used the same Adam, full CUEQ, TF32, AMP-none, WSD, data split, seed, and model settings as `621462`, but without `--edge_force_compile`. The eager run completed all `200450` updates; hot-step mean was `26.41 ms/batch` and median was `22.92 ms/batch`. Final metrics were train `51.6` meV/atom / `155.3` meV/A, valid `45.7` / `227.6`, and test `51.3` / `158.9`. `nvdmon` showed about `22.8%` average SM utilization and about `2.49 GB` framebuffer use. Against this strict eager control, full-Inductor edge-force compile gives `1.89x` end-to-end speedup (`01:40:51` -> `00:53:22`) and about `2.07x` mean hot-step speedup (`26.41` -> `12.77 ms/batch`) without degrading final force accuracy at displayed precision. Memory remains the tradeoff: Slurm MaxRSS increased from `3.58 GB` to `4.95 GB`, while framebuffer use was essentially unchanged in `nvdmon`.
- The matching conservative HybridMuon RECIO L1/C64 200k WSD compile gate, SAI job `622121`, completed in `01:08:50` with MaxRSS `4.95 GB`. It used the same full CUEQ, full Inductor edge-force compile, TF32, AMP-none, data split, seed, and model settings as `621462`, but with `--optimizer=hybrid_muon --hybrid_muon_mode=2d --hybrid_muon_routing=mace --hybrid_muon_lr_factor=0.1 --hybrid_muon_weight_decay=0.0`. Routing sent `8` tensors / `74,752` parameters through Muon and `14` tensors / `160,784` parameters through Adam. Compile health stayed clean over all `211` epochs: one initial compile, `fallbacks=0`, `runtime_recompiles=0`, hot-step mean `16.83 ms/batch`, median `14.45 ms/batch`, max energy/force parity difference `3.052e-5` / `1.907e-5`, and max parameter-gradient difference `1.312e-3`. Final metrics were train `49.6` meV/atom / `154.9` meV/A, valid `45.7` / `227.5`, and test `49.4` / `158.5`. `nvdmon` showed about `21.6%` average SM utilization and about `2.46 GB` framebuffer use. This proves conservative HybridMuon is compatible with CUEQ plus full-Inductor force-backward compile on a real 8k RECIO run and does not hurt final force accuracy at displayed precision. It is not the fastest L1/C64 recipe: compared with Adam compile, wall time regressed by `1.29x` (`00:53:22` -> `01:08:50`) and mean hot-step time regressed by `1.32x` (`12.77` -> `16.83 ms/batch`). Use it for optimizer ablations, not as the current default throughput recipe.
- The longer OC20NEB FPS L1/C64 200k WSD gate gives the best current cross-dataset evidence for the full CUEQ plus full-Inductor force-backward path. Adam compile, SAI job `622279`, completed in `01:08:02` with MaxRSS `8.91 GB`; the matched Adam eager control, SAI job `622280`, completed in `01:40:34` with MaxRSS `6.07 GB`. Both used the DeepMD OC20NEB FPS extxyz split, `ScaleShiftMACE`, `max_L=1`, `num_channels=64`, `correlation=3`, batch size `8`, full CUEQ, TF32, AMP none, WSD, and seed `456`. Compile health was clean for all `320` epochs: one initial compile, `fallbacks=0`, `runtime_recompiles=0`, hot-step mean `14.69 ms/batch`, median `14.59 ms/batch`, max energy/force parity difference `3.662e-4` / `5.722e-6`, and max parameter-gradient difference `2.953e-5`. Eager hot-step mean/median were `24.72` / `24.63 ms/batch`, so the hot-step speedup is `1.68x`; strict Slurm wall-clock speedup is `1.48x` (`01:40:34` -> `01:08:02`). Final metrics were essentially matched: compile train `16.5` meV/atom / `51.5` meV/A, valid `18.2` / `43.3`, test `18.2` / `43.3`; eager train `16.6` / `51.1`, valid `18.7` / `42.9`, test `18.7` / `42.9`. This is a stronger OC20NEB result than the earlier 20k smoke: compile gives a real end-to-end gain after amortization, but still not DPA4's reported `~3x` system speedup on this V100/MACE stack, and it increases CPU MaxRSS by about `2.84 GB`.
- The matching conservative HybridMuon OC20NEB compile run, SAI job `622304`, was intentionally stopped after epoch `101` / `63,223` updates because the first real validation point showed no learning. It used the same full CUEQ, full-Inductor compile, TF32, AMP-none, seed, data split, and model as `622279`, but with `--optimizer=hybrid_muon --hybrid_muon_mode=2d --hybrid_muon_routing=mace --hybrid_muon_lr_factor=0.1 --hybrid_muon_weight_decay=0.0`. Engineering compatibility was fine: one initial compile, `fallbacks=0`, `runtime_recompiles=0`, hot-step mean `15.33 ms/batch`, median `15.18 ms/batch`, and MaxRSS `6.72 GB`. Accuracy was not fine: epoch-80 validation stayed at `156.5` meV/atom / `23.7` meV/A, essentially unchanged from the initial `157.1` / `23.9`, while Adam had already reached about `20.3` / `50.0` at epoch 80. Treat this as a real negative result for the current conservative HybridMuon recipe on OC20NEB FPS, not as a compile incompatibility. Future Muon work should revisit routing/lr/schedule against TACE/DPA4 before spending full 200k budgets on this dataset.
- Follow-up diagnostics isolated two separate HybridMuon issues on OC20NEB FPS. First, job `622433` repeated the L1/C64 compile setup with `WEIGHT_DECAY=0.0` for `32` epochs and did learn: at epoch `16`, validation reached `34.2` meV/atom / `56.6` meV/A, and gradients stayed healthy instead of collapsing. Second, the HybridMuon implementation was fixed to preserve MACE's original Adam fallback decay/no-decay parameter groups, because applying global `weight_decay=0.001` to all Adam-routed singleton/readout/embedding parameters could shrink sensitive weights nearly to zero. After that fix, job `622461` reran the same `32` epoch L1/C64 full-CUEQ/full-Inductor/TF32 HybridMuon case with normal `WEIGHT_DECAY=0.001`. Compile health was excellent over all `20,000` updates: one initial compile, `19,999` cache hits, `fallbacks=0`, `runtime_recompiles=0`, hot median step time about `15-16 ms/batch`, MaxRSS `7.77 GB`, and parity checks accepted with max energy differences up to `3.36e-4` and max gradient differences around `7.0e-8`. Optimizer quality was still poor: loss median only moved from about `2.92` to `2.79`, epoch-16 validation stayed `158.5` meV/atom / `23.7` meV/A, and final evaluation loaded the epoch-0 checkpoint with valid/test `157.4` / `23.8`. This means the current OC20NEB HybridMuon problem is no longer a compile/CUEQ compatibility issue or a hard gradient-collapse bug; it is a recipe/routing/decay issue. The matched post-fix `WEIGHT_DECAY=0.0` rerun, job `622492`, completed in `00:15:13` with MaxRSS `7.91 GB`; it reproduced the earlier learning behavior and kept compile health clean for all `20,000` updates, with one initial compile, `fallbacks=0`, and `runtime_recompiles=0`. Its best checkpoint was epoch `16`, with train `24.4` meV/atom / `77.4` meV/A and valid/test `34.2` / `56.6`. This makes `WEIGHT_DECAY=0.0` the safer OC20NEB diagnostic setting until a better Muon recipe is proven.
- The next controlled OC20NEB HybridMuon routing check, SAI job `622508`, used the same L1/C64, full CUEQ, full-Inductor force compile, TF32, AMP none, seed `456`, `WEIGHT_DECAY=0.0`, and `hybrid_muon_lr_factor=0.1`, but with `--hybrid_muon_routing=tace --hybrid_muon_mode=slice`. This increased Muon coverage from `8` tensors / `74,752` parameters to `10` tensors / `352,000` parameters by moving `products.0.symmetric_contractions.weight` and `products.1.symmetric_contractions.weight` onto per-slice Muon with `38` independent slices each. Engineering compatibility was clean: the job completed in `00:15:34` with MaxRSS `7.84 GB`, one initial compile, `fallbacks=0`, `runtime_recompiles=0`, and accepted parity checks through epoch `31` with max energy/force/gradient differences `6.104e-5` / `6.676e-6` / `1.520e-6`. Optimizer quality did not improve: best checkpoint epoch `16` gave train `28.4` meV/atom / `83.5` meV/A and valid/test `41.9` / `59.2`, worse than conservative `routing=mace` job `622492` (`34.2` / `56.6`) and far worse than Adam `622279` (`18.2` / `43.3`). Treat direct TACE-style slice routing of MACE `symmetric_contractions.weight` as a negative result at this lr/decay setting; future Muon work should change the recipe or block design before expanding coverage further.
