...
Two formal OC20NEB L=1/C64 jobs completed from `runs/oc20neb_fps_l1c64_wsd_20k_20260703`: eager CUEQ Adam WSD job `621339` and full-Inductor CUEQ Adam WSD job `621340`. Both used `batch_size=8`, `max_num_epochs=32` (exactly `20000` optimizer updates), independent `valid.extxyz`, TF32 enabled, AMP disabled, full CUEQ, and identical seed/schedule.

| Mode | Job | Slurm elapsed | MaxRSS | Final valid E/F | Final test E/F | Compile health |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| CUEQ eager | `621339` | `00:16:07` | `5802996K` | `31.0` meV/atom / `54.1` meV/A | `31.0` meV/atom / `54.1` meV/A | n/a |
| CUEQ + full Inductor edge-force compile | `621340` | `00:14:16` | `7720504K` | `31.0` meV/atom / `54.1` meV/A | `31.0` meV/atom / `54.1` meV/A | `fallbacks=0`, `runtime_recompiles=0`, parity and fixed-probe checks accepted |

This OC20NEB result is an important negative/bounded-speedup control. Full Inductor compile is compatible with the DPA4 FPS split, CUEQ, TF32, WSD, and a broad element set; it preserves the validation/test metrics exactly at the displayed precision. However, for this L=1/C64 model the end-to-end speedup is only about `1.13x`, while MaxRSS rises from about `5.80 GB` to `7.72 GB`. The hot training epochs after setup are faster in the compile log (`~8.7-9.4 s` for 625 steps, after the initial strict setup), but validation, final testing, checkpoint conversion, Python overhead, and the small model size limit end-to-end gain. This aligns with the RECIO evidence: compile is most useful once model size or compiled force/backward work is large enough. The important SAI submission lesson from the smoke setup remains that explicit `sbatch --export=ALL,...` caused immediate `CANCELLED by 0` jobs with no stdout/stderr on this cluster. Environment-prefix submission, for example `RUN_ROOT=... EDGE_FORCE_COMPILE=True sbatch scripts/benchmarks/oc20neb_fps/run_mace_oc20neb_fps_sai.sh`, works and should be used for these wrappers.
